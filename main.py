...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/spending_tip':
            ...
            self.end_headers()
            dict = {"value": llm.get_spending_tip()}
            ...
            self.send_header("Content-type", "application/json")
            self.end_headers()
            dict = {"value": llm.get_category_tip()}
            json_data = json.dumps(dict)
            self.wfile.write(json_data.encode("utf-8"))
    ...
    def do_POST(self):
        if self.path == "/category":
            ...
            category_result = llm.get_category(data_string)
            ...
            category_result = llm.add(data_string)
            ...
            category_result = llm.chat(data_string)
